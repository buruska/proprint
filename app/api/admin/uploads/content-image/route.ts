...
const CONTENT_IMAGE_BUCKET: UploadBucketName = "content-images";
...
export async function POST(request: Request) {
  ...
  if (access.status === "forbidden") {
    return NextResponse.json(
      { message: "Nincs jogosultságod az oldal képeinek feltöltéséhez." },
      { status: 403 },
    );
  }
  ...
  if (file.size > MAX_FILE_SIZE) {
    return NextResponse.json(
      { message: "Az oldalba szúrt kép legfeljebb 8 MB méretű lehet." },
      { status: 400 },
    );
  }
  ...
  return NextResponse.json({
    message: "A kép sikeresen feltöltve és beszúrható az oldalba.",
